...
var Cursor = module.exports = function(documents, opts) {
  ...
  function getDocs(applySkipLimit) {
    ...
    docs = docs.map(convertBufferToHexString);
    if (opts.sort) {
  ...
  var interface = {
    ...
    next: function (callback) {
      ...
      doc = convertBufferToHexString(doc);
      if(typeof callback !== 'function')
    ...
    toArray: function (callback) {
      ...
      function done() {
        interface.rewind();
        console.log('mongo-mock one called');
        return getDocs(true).map(convertBufferToHexString);
      }
